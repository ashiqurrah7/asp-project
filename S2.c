 ...
 * S2.c (Option B fix)
 *
 *  - Listens on port 6002 for "STORE", "GET", "REMOVE", "TAR", "LIST"
 *  - If the path from S1 ends in '/', or is something like "/",
 *    we auto-append "default.pdf" so we don't get "Is a directory."
 *  - That means the final file is stored as e.g. "S2//default.pdf"
 *    or "S2/myfolder/default.pdf".
 *
 *  NOTE: This loses the original filename if S1 didn't include it
 *  in the path. If you *do* want the original name, you'd have S1
 *  pass "folder/hello.pdf" to S2. But this code at least avoids
 ...
 *         3) file data
 *
 *   - We convert "~S1/folder" => "S2/folder"
 *   - If the final path ends with '/', we append "default.pdf"
...
void cmd_STORE(int cfd) {
  ...
  // 3) Convert path from "~S1/..." => "S2/..."
  char localpath[1024];
  memset(localpath, 0, sizeof(localpath));

  // e.g. "~S1/folder/testdir" => "S2/folder/testdir"
  ...
  // If localpath ends in '/', or if it's "S2/", we have no filename =>
  // let's auto-append "default.pdf"
  ...
  // e.g. if user said path="/", then localpath = "S2//" => we fix by appending
  // "default.pdf"
  // => "S2//default.pdf"
...
/***************************************************************
 * cmd_GET: S1 requests a file path, we read from S2, send size + data
 ***************************************************************/
void cmd_GET(int cfd) {
  char *path = recv_string(cfd);
  if (!path)
  ...
  // e.g. path "~S1/folder/abc.pdf" => "S2/folder/abc.pdf"
  char localpath[1024];
  if (strncmp(path, "~S1/", 4) == 0) {
    snprintf(localpath, sizeof(localpath), "%s/%s", BASE_FOLDER, path + 4);
  } else {
...
/***************************************************************
 * cmd_TAR:
 *   - We'll tar up everything under S2
 ***************************************************************/
void cmd_TAR(int cfd) {
...
/***************************************************************
 * cmd_LIST:
 *   - S1 wants a directory listing under S2
 ***************************************************************/
void cmd_LIST(int cfd) {
  char *path = recv_string(cfd);
  if (!path)
