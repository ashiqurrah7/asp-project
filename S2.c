...
int main() {
    ...
    serv_addr.sin_port = htons(PORT_S2);
    ...
    printf("S2 server (PDF) running on port %d\n", PORT_S2);
    while(1) {
        ...
        if(strncmp(command, "uploadf", 7) == 0) {
            ...
            char fullPath[512];
            snprintf(fullPath, sizeof(fullPath), "S2%s", dest_path+3);
            ...
            char msg[] = "File stored on S2.\n";
            send(newsockfd, msg, strlen(msg), 0);
        } else if(strncmp(command, "downlf", 6) == 0) {
            char filepath[256];
            sscanf(command, "downlf %s", filepath);
            char localPath[512];
            snprintf(localPath, sizeof(localPath), "S2%s", filepath+3);
            FILE *fp = fopen(localPath, "rb");
            if(fp == NULL) {
                char msg[] = "File not found on S2.\n";
                send(newsockfd, msg, strlen(msg), 0);
                close(newsockfd);
                continue;
            }
            ...
            if(remove(localPath)==0)
                send(newsockfd, "File removed from S2.\n", 23, 0);
            else
                send(newsockfd, "Error removing file from S2.\n", 29, 0);
        } else if(strncmp(command, "downltar", 8) == 0) {
            char filetype[16];
            sscanf(command, "downltar %s", filetype);
            if(strcmp(filetype, ".pdf") == 0) {
                system("tar -cf pdffiles.tar S2");
                FILE *fp = fopen("pdffiles.tar", "rb");
                if(fp == NULL) {
                ...
                send(newsockfd, fsizeStr, strlen(fsizeStr), 0);
                sleep(1);
                send_file(newsockfd, "pdffiles.tar");
                fclose(fp);
                remove("pdffiles.tar");
            }
            ...
            char localPath[512];
            snprintf(localPath, sizeof(localPath), "S2%s", pathname+3);
