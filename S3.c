...
int main(){
    ...
    serv_addr.sin_port=htons(PORT_S3);
    ...
    printf("S3 server (TXT) running on port %d\n", PORT_S3);
    while(1){
        ...
        if(strncmp(command,"uploadf",7)==0){
            ...
            char fullPath[512];
            snprintf(fullPath, sizeof(fullPath), "S3%s", dest_path+3);
            ...
            send(newsockfd, "File stored on S3.\n", 20, 0);
        } else if(strncmp(command,"downlf",6)==0){
            char filepath[256];
            sscanf(command, "downlf %s", filepath);
            char localPath[512];
            snprintf(localPath, sizeof(localPath), "S3%s", filepath+3);
            FILE *fp = fopen(localPath,"rb");
            if(fp==NULL){
                send(newsockfd, "File not found on S3.\n", 23, 0);
                close(newsockfd);
                continue;
            }
            fseek(fp, 0, SEEK_END);
            ...
            if(remove(localPath)==0)
                send(newsockfd, "File removed from S3.\n", 23, 0);
            else
                send(newsockfd, "Error removing file from S3.\n", 29, 0);
        } else if(strncmp(command,"downltar",8)==0){
            char filetype[16];
            sscanf(command,"downltar %s", filetype);
            if(strcmp(filetype, ".txt")==0){
                system("tar -cf txtfiles.tar S3");
                FILE *fp = fopen("txtfiles.tar", "rb");
                if(fp==NULL){
                    send(newsockfd, "Error creating tar file.\n", 26, 0);
                    close(newsockfd);
                    continue;
                }
                fseek(fp, 0, SEEK_END);
                ...
                send(newsockfd, fsizeStr, strlen(fsizeStr), 0);
                sleep(1);
                send_file(newsockfd, "txtfiles.tar");
                fclose(fp);
                remove("txtfiles.tar");
            }
            ...
            char localPath[512];
            snprintf(localPath, sizeof(localPath), "S3%s", pathname+3);
